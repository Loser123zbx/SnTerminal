...
class mainWindow(wx.Frame):
    ...
    def __init__(self, parent):
        wx.Frame.__init__(self, parent, id=wx.ID_ANY, title=wx.EmptyString, pos=wx.DefaultPosition,
                          size=wx.Size(862, 460), style=wx.DEFAULT_FRAME_STYLE | wx.TAB_TRAVERSAL)
        ...
        self.m_genericDirCtrl1 = wx.GenericDirCtrl(self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition,
                                                   wx.DefaultSize, wx.DIRCTRL_3D_INTERNAL | wx.SUNKEN_BORDER,
                                                   wx.EmptyString, 0)
...
class work_place(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.Size(800, 460),
                 style=wx.TAB_TRAVERSAL, name=wx.EmptyString,
                 terminal_name=f"terminal-{terminal.get_time()}", terminal_path="C:\\"):
        wx.Panel.__init__(self, parent, id=id, pos=pos, size=size, style=style, name=name)
        self.terminal_name = terminal_name
        self.terminal_path = terminal_path
        self.terminal = terminal.terminal(terminal_path)
        self.terminal.welcome()
        self.terminal_status = "Free"

        workPlace = wx.BoxSizer(wx.VERTICAL)

        self.output = wx.richtext.RichTextCtrl(self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition,
                                               wx.DefaultSize, 0 | wx.VSCROLL | wx.HSCROLL | wx.NO_BORDER | wx.WANTS_CHARS)
        self.output.SetFont(wx.Font(wx.NORMAL_FONT.GetPointSize(), wx.FONTFAMILY_DEFAULT,
                                    wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL, False, wx.EmptyString))

        workPlace.Add(self.output, 1, wx.EXPAND, 5)

        self.input = wx.TextCtrl(self, wx.ID_ANY, wx.EmptyString, wx.DefaultPosition,
                                 wx.DefaultSize, wx.TE_PROCESS_ENTER)
        workPlace.Add(self.input, 0, wx.EXPAND, 5)

        self.information = wx.StaticText(self, wx.ID_ANY,
                                         (f"执行目录：{terminal_path}；状态：{self.terminal_status}；"),
        ...
        # 修复：注册当前实例的 terminal 对象，而不是新建一个
        terminals[terminal_name] = self.terminal

    def OnEnter(self, event):
        command = self.input.GetValue()
        self.output.AppendText(command + "\n")
        self.terminal_status = "Running"
        # 修复：使用 self.terminal_path
        self.information.SetLabel(f"执行目录：{self.terminal_path}；状态：{self.terminal_status}；")
        self.terminal.handle_command(command)
...
class command_history(wx.Panel):
    ...
    def __init__(self, parent, id=wx.ID_ANY, pos=wx.DefaultPosition, size=wx.Size(800, 460),
        ...
        command_listChoices = []
        self.command_list = wx.ListBox(self, wx.ID_ANY, wx.DefaultPosition, wx.DefaultSize,
                                       command_listChoices, wx.LB_ALWAYS_SB | wx.LB_MULTIPLE)
